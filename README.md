...
계단형 지형에서 장애물 주변 노드가 불필요하게 계속 생성되는 문제가 발생했습니다.  

- 원인

장애물 인접 구간에서 대각선으로 노드를 확장할 때,  
코너(Corner) 충돌 여부를 판단하는 로직이 부족했습니다.

- 해결

대각선으로 확장하기 전, 우회 가능 여부를 검사하는 조건을 추가하여  
장애물을 끼고 돌아갈 수 없는 경우에는 대각선 확장을 막도록 수정하였습니다.
...
이미 탐색이 끝난 위치가 다시 생성되면서,  
동일 지점을 반복 탐색하는 루프가 발생했습니다.

- 해결
  
closeList를 두어 한 번 사용된 노드의 좌표를 기록하고,  
이미 방문한 좌표는 이후 탐색 대상에서 제외하도록 처리했습니다.

- 결과
  
불필요한 재탐색이 제거되어 루프가 사라졌고,  
알고리즘이 정상적으로 동작하며 탐색의 안정성과 효율성을 확보할 수 있었습니다.
